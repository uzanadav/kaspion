...
select
    ...
    budget_ils,
    budget_is_suggested,
    round(coalesce(budget_ils, 0) * month_fraction, 2)               as budget_to_date_ils,
    round(coalesce(budget_ils, 0) * month_fraction - actual_ils, 2)  as pace_ils,
    case
        when budget_ils is null then 'no_budget'
        when actual_ils <= budget_ils * month_fraction then 'under'
        else 'over'
    end as pace_status
from resolved
